...
async function geocodePostcode(postcode) {
    ...
    for (let i = 1; i <= 2; i++) {
        ...
        if (result) {
            console.log(`Successfully geocoded alternative postcode ${altPostcode} for ${postcode}`);
            return result;
        }
    }
...
async function geocodeLocationName(locationName) {
    try {
        ...
        const response = await fetch(`https://api.postcodes.io/postcodes?q=${encodeURIComponent(searchQuery)}`);
...
async function processGym(gym, browser, retries = 3) {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        ...
        try {
            ...
            const geocodeResult = postcode ? await geocodePostcode(postcode) : await geocodeLocationName(gym.name);
    ...
    try {
        ...
        for (let i = 0; i < activeGyms.length; i += batchSize) {
            ...
            for (const gym of batch) {
                const progress = ((i + batch.indexOf(gym) + 1) / activeGyms.length * 100).toFixed(1);
                try {
                    const result = await processGym(gym, browser);
                    results.push(result);
                    console.log(`[${progress}%] Processed: ${gym.name} - £${result.price} - ${result.latitude ? `Geocoded (${result.geocodeSource})` : 'No coordinates'}`);
                } catch (error) {
            ...
            console.log(`Progress saved: ${results.length}/${activeGyms.length} gyms processed`);
